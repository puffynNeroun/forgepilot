...
import { ReleaseTimeline, ReleaseTimelineEmptyState } from "../../components/releases/ReleaseTimeline";
import { getReleaseTimelineData } from "../../lib/db/releases";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

export default async function ReleasesPage() {
  const data = await getReleaseTimelineData();
    ...
    return (
      <main className="min-h-screen bg-slate-950 px-6 py-10 text-slate-100">
        <div className="mx-auto max-w-3xl rounded-3xl border border-rose-500/30 bg-rose-500/10 p-8">
          ...
          <h1 className="mt-6 text-3xl font-semibold text-white">Release timeline unavailable</h1>
          <p className="mt-4 text-sm leading-6 text-rose-100/90">
            ForgePilot could not load releases from the local database.
          </p>
        ...
        <div className="mx-auto flex w-full max-w-5xl flex-col gap-8">
          <header className="rounded-3xl border border-white/10 bg-white/[0.03] p-8">
            ...
            <p className="mt-4 text-base leading-7 text-slate-300">
              The ForgePilot product exists, but there are no releases to display yet.
            </p>
          </header>
  ...
  return <ReleaseTimeline data={data} />;
}
